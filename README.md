...
| 指标 | v1 | v2 |
|------|-----|-----|
| RMSLE | 0.39970 | Pending |
| 模型 | Stacking (XGB+LGB+RF) | 自动选择最优（调参后 XGB/LGB vs Stacking） |
| CV 策略 | TimeSeriesSplit(5) | TimeSeriesSplit(5) |
| 特征数 | 19 | 26（+6 循环编码 + 新交互） |
| 预测策略 | 分头预测 | 分头预测 + 自动模型选择 |
| 关键改进 | — | 循环编码 / atemp 替换 temp / weather=4 去噪 / 搜索加倍 |

v2 比 v1 的核心变化：hour/month/weekday 做了 sin/cos 循环编码，让模型正确理解时间循环性；用 atemp（体感温度）替代 temp；修复了 v1 模型选择逻辑的 bug（v1 盲目用 Stacking，v2 自动对比所有策略选最优）。具体迭代过程写在 Notebook 第 8 章。
